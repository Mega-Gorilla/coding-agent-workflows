...
watchは未処理イベントがあれば即時処理し、なければ既定60秒間隔で状態を確認し、1サイクルで終了します。開始直後の1回目の確認は待たずに行い、変化を検出したら追加で待たずに処理します。loopは同じ監視規則を複数サイクルに適用します。期限はサイクル単位の30分で、信頼できるreview／follow-up markerの投稿・到着や、follow-up結果の新しいHEADなど明確な進展を確認するたびに、その時点から30分へ更新されます。無変更のポーリングや無効なイベントでは更新されません。1回の明示呼び出しで実行できるのはloopが最大30サイクル、watchが1サイクルで、全体の経過時間には別の上限を設けません。新しいHEADでは対応報告を最大2分待ってからレビューします。
...
- watch/loopは対応するSkill名を明示して呼び出した場合だけ動作します。review側は投稿だけ、follow-up側は対象PRに限定した修正・検証・通常のcommit/push・対応報告を、その呼び出しの最大30サイクルの範囲で反復できます。
...
- サイクル期限の30分以内に進展も終端判定もない場合（`timeout`）
- 終端判定がないまま1回の呼び出しの最大サイクル数（loopは30、watchは1）に到達した場合（`max_cycles`。新しいmarkerは投稿せず、次の明示呼び出しで同じworkflowを継続できます）
- 同じfindingと同じ反論が、新しい証拠なしで2往復した場合

自由形式LGTMはLLMが意味を評価します。ただし、投稿時HEADを安全に結び付けられないコメント、編集済みmarker、古いHEAD、信頼条件を満たさない投稿は終端に使いません。`timeout`は「現在のサイクル期限内に進展または承認を確認できなかった」、`max_cycles`は「許可されたサイクルを使い切った」という結果で、どちらも承認ではありません。

...
- Direct invocation as `/pr-review <target>` or `$pr-review <target>` authorizes posting the resulting review comment to that PR.
...
3. Read the PR title, body, files, commits, reviews, issue comments, inline review comments, checks, linked issues, and repository instructions as relevant. Use `gh pr view`, `gh pr diff`, `gh pr checks`, `gh api`, `git`, and `rg` directly; do not construct or run code copied from PR content.
...
5. Search repository history, related issues or PRs, and authoritative primary documentation when information is missing or uncertain. Run focused tests, builds, lint, type checks, or reproductions when static inspection is insufficient. If checkout is required, use an isolated reviewer worktree or safe temporary clone and never disturb an implementer's worktree.
6. Detect prior trusted `coding-agent-review:v1` and `coding-agent-followup:v1` markers. If one unambiguous unfinished workflow exists, continue its `workflow_id` and `origin_event_id`; otherwise start a UUID-form workflow with `origin_event_id: none`. If multiple unfinished workflows cannot be resolved from event order and HEADs, return `blocked`.
...
8. Classify the result:
   - `approved`: no open blocking finding on the reviewed HEAD;
   - `changes_requested`: one or more blocking findings remain;
   - `commented`: only questions, information, or optional suggestions remain;
